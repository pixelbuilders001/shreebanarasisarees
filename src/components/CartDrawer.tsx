...
export const CartDrawer: React.FC = () => {
  const { cart, isCartOpen, setIsCartOpen, updateCartQuantity, removeFromCart } = useStore();
    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        ...
        <div className="w-screen max-w-md bg-[#FFF9F0] border-l border-gold/20 flex flex-col shadow-2xl animate-slide-in">
          {/* Header */}
          <div className="px-4 py-5 bg-[#FFFFFF] border-b border-cream flex items-center justify-between">
            <h2 className="text-lg font-serif font-bold text-dark-brown flex items-center gap-2">
              ...
              <div className="space-y-4">
                {cart.map((item) => {
                  ...
                  return (
                    <div 
                      key={item.product.id}
                      className="bg-white p-3 rounded-lg border border-cream flex gap-3 shadow-sm"
                    >
                      {/* Product Thumbnail */}
                      <div className="w-20 aspect-[3/4] rounded overflow-hidden bg-cream/30 flex-shrink-0">
                        <img
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-start">
                            <h4 className="text-sm font-serif font-bold text-dark-brown line-clamp-1">
                              {item.product.name}
                            </h4>
                            <button
                              onClick={() => removeFromCart(item.product.id)}
                              className="text-dark-brown/40 hover:text-red-600 transition-colors ml-2"
                        ...
                        <div className="flex justify-between items-center mt-2">
                          ...
                          <div className="flex items-center border border-cream rounded bg-cream/20">
                            <button
                              ...
                              className="p-1 text-dark-brown/60 hover:text-maroon hover:bg-cream/40 transition-colors"
                              aria-label="Decrease quantity"
                            >
                              <Minus size={12} />
                            </button>
                            <span className="px-2 text-xs font-semibold text-dark-brown min-w-[20px] text-center">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)}
          ...
          {/* Footer Summary */}
          {cart.length > 0 && (
            <div className="bg-[#FFFFFF] border-t border-cream px-4 py-6 sm:px-6">
              <div className="space-y-2">
                <div className="flex justify-between text-sm text-dark-brown/70 font-medium">
                  <span>Subtotal</span>
                ...
                <div className="flex justify-between text-base font-serif font-bold text-dark-brown border-t border-cream pt-2.5 mt-1">
                  <span>Total</span>
              ...
              <div className="mt-6 flex flex-col gap-3">
                <Link
                  href="/checkout"
                  onClick={() => setIsCartOpen(false)}
                  className="w-full py-3 bg-maroon text-ivory text-center rounded font-semibold tracking-wide hover:bg-maroon-dark hover:scale-[1.01] active:scale-[0.99] transition-all shadow-md text-sm uppercase"
                >
                  PROCEED TO CHECKOUT
                </Link>
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="w-full text-center text-xs text-dark-brown/65 hover:text-maroon font-medium transition-colors"
                >
                  Continue Shopping
                </button>
              </div>
